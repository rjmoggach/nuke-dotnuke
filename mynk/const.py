...
def const(const):
    ...
    try:
        ...
        LOG.error(errno.EINVAL, "No such constant: {0}".format(const))
    except TypeError:
        LOG.error(
            errno.EINVAL,
            "const name must be a string or unicode object, not: {0}".format(
...
def set_const(const, val):
    ...
    try:
        ...
        LOG.error(errno.ENOENT, "no such constant: {0}".format(const))
    except TypeError:
        LOG.error(
            errno.EINVAL,
            "const name must be a string or unicode object, not: {0}".format(
