...
__menus__ = {
    "Create/Write From Read": {
        "cmd": "make_write_from_read()",
        "hotkey": "#w",
        "icon": "",
    }
}
...
def make_write_from_read():
    ...
    Typical scenario:
            1. Drop in a munch of still images.
    ...
    exrCompressionList = []
    exrCompressionList.append('"DWAA"')  # good grainy images
    ...
    exrCompression = []
    exrCompression.append('"25"')  # good grainy images
    exrCompression.append('"35"')  # good grainy images
    exrCompression.append('"45"')  # good grainy images
    ...
    exrCompressionTypePulldown = " ".join(exrCompressionList)
    exrCompressionPulldown = " ".join(exrCompression)
    ...
    exrCompressionTypePulldownHandle = "EXR Compression:"
    exrCompressionPulldownHandle = "EXR Compression Amt:"
    ...
    p.addEnumerationPulldown(
        exrCompressionTypePulldownHandle, exrCompressionTypePulldown
    )
    ...
    selectedExrCompressionType = p.value(exrCompressionTypePulldownHandle)
    ...
    if (addPath == "") and (addPrefix == "") and (addPostfix == ""):
        if selectedFormat == "nochange":
            result = nuke.ask(
                "Warning, your write nodes will have the same filename as\nyour Read-nodes.\n\n Are you sure that this is what you want?"
            )
        else:
            result = nuke.ask(
                "Warning, you only changed the file format.\n\nYour write nodes MIGHT have the same filename as\nyour Read-nodes.\n\n Are you sure that this is what you want?"
            )
        #
        if result:
            pass
    ...
    selectThese = []
    for tmpNode, readNode in readNodeLut.items():
        ...
        if selectedFormat == "nochange":
            ...
            if (tmpExt.lower() == "jpg") or (tmpExt.lower() == "jpeg"):
                ...
                (tmpExt.lower() == "tif")
                or (tmpExt.lower() == "tiff")
                or (tmpExt.lower() == "tif16")
            ):
                type = "tiff"
            else:
                type = tmpExt
        ...
        newFilename = os.path.join(
            newFilename, addPrefix + basefile + addPostfix + extension
        ).replace("\\", "/")
