...
KNOBS_3D = ["translate", "rotate", "scaling", "skew", "pivot", "near"]

KNOBS_UV = ["win_translate", "rotate", "scaling", "skew", "pivot", "near"]

ROTATE_CHANS = ("rotate.x", "rotate.y", "rotate.z")

DATA_FORMAT_OPTIONS = "mm cm inch"

CAMERA_KNOBS = ["haperture", "vaperture", "focal"]
    ...
    if result:
        ...
        if "haperture" in conversions:
            if p.value("haperture") == "inch":
                hapertureConversion = 25.4
            elif p.value("haperture") == "cm":
                hapertureConversion = 100
            else:
                hapertureConversion = 1
        else:
            hapertureConversion = 1

        if "vaperture" in conversions:
            if p.value("vaperture") == "inch":
                vapertureConversion = 25.4
            elif p.value("vaperture") == "cm":
                vapertureConversion = 100
            else:
                vapertureConversion = 1
        else:
            vapertureConversion = 1
...
def createChannelOrderPanel(dataTypesCount, dataTypes=""):
    ...
    for chanNumber in range(dataTypesCount):
        p.addEnumerationPulldown(str(chanNumber), str.join(dataTypes))
...
def importChan(chanFile=""):
    ...
    (
        ...
        hapertureConversion,
        vapertureConversion,
    ) = getDataConversions(knobLookups)
    ...
    try:
        frameDataLocation = knobLookups.index("frame")
    except:
        frameDataLocation = -1
    ...
    for line in fileLines:
        ...
        for lineNumber in range(len(lineData)):
            ...
            if knob in ["none", "frame"]:
                print(
                    "PASS(%s,%s) %s %s"
                    % (str(frame), str(lineNumber), knob, str(value))
                )
            else:
