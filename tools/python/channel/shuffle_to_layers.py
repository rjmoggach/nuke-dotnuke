...
__menus__ = {
    "Channel/Shuffle Layers": {
        "cmd": "shuffle_to_layers(nuke.selectedNodes())",
        "hotkey": "",
        "icon": "",
    }
}
...
def shuffle_to_layers(nodes, margins=[70, 30], dots=True):
    """
    ...
    for node in nodes:
        ...
        if not node_xywh:
            ...
            node.setXYpos(
                int(shuf_xywh[0] + (shuf_xywh[2] + dot_xywh[2]) / 2 + margins[0]),
                int(dot_xywh[1] - margins[1] - node_xywh[3]),
            )
        layers = []
        ...
        for each in layers:
            ...
            if not dot_xywh:
                ...
                if not shuf_xywh:
                    dot.setXYpos(
                        int(node_xywh[0] + (node_xywh[2] - dot_xywh[2]) / 2),
                        int(node_xywh[1] + node_xywh[3] + margins[1]),
                    )
                else:
                    dot.setXYpos(
                        int(shuf_xywh[0] + margins[0] + shuf_xywh[2]),
                        int(shuf_xywh[1] - margins[1] - dot_xywh[3]),
                    )
            else:
                dot.setInput(0, nuke.toNode(dotName))
                dot.setXYpos(
                    int(dot_xywh[0] + shuffle.screenWidth() + margins[0]),
                    int(dot_xywh[1]),
                )
            ...
            shuffle.setXYpos(
                int(dot_xywh[0] - (shuffle.screenWidth() - dot_xywh[2]) / 2),
                int(dot_xywh[1] + dot_xywh[3] + margins[1]),
            )
            shuf_xywh = [
                shuffle.xpos(),
                shuffle.ypos(),
                shuffle.screenWidth(),
                shuffle.screenHeight(),
            ]
