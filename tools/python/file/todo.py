...
from PySide2 import QtGui, QtCore, QtWidgets, QtNetwork
...
class NukeError(Exception):
    def __str__(self):
        return 'This is the "A PythonObject is not attached to a node" error we need to work around'
...
class Task(object):
    ...
    def __str__(self):
        return "-" * 20 + "\np%s:                %s                 (%s)" % (
            self.priority,
            self.name,
            ["waiting", "in progress", "finished"][self.status],
        )
...
class TaskStore(QtCore.QObject):
    ...
    def sortByPriority(self, active):
        ...
        sortedTasks = sorted(
            [t for t in self.tasks if t.index >= 0], key=lambda task: task.priority
        )
        if active:
            # sort highest first
            sortedTasks.reverse()
...
class DragIndicator(QtWidgets.QWidget):
    ...
    def sizeHint(self):
        return QtCore.QSize(
            self.parentWidget().width(), self.parentWidget().height() / 6
        )
    ...
    def paintEvent(self, event):
        ...
        gradient = QtGui.QLinearGradient(
            QtCore.QPoint(0, 0), QtCore.QPoint(self.width() / 2, 0)
        )
...
class TaskWidget(QtWidgets.QWidget):
    ...
    def getNewPosition(self):
        ...
        if self.task.index >= 0:
            ...
            y = (
                self.task.index
                * TaskWidget.TASKWIDGETHEIGHT
                * TaskWidget.TASKWIDGETSPACING
            )
...
class PriorityWidget(QtWidgets.QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self.setToolTip(
            "<b>priority</b><br>use either:<ul><li>LMB to increase    -    RMB to decrease</li><li>alt+LMB drag to change value</li><li>MMB drag to change value</li></ul><i>move mouse away after changing value<br>to trigger re-sorting</i>"
        )
        self.active = False
...
class StatusWidgetPie(QtWidgets.QComboBox):
    def __init__(self, parent=None):
...
class StatusWidgetBar(QtWidgets.QComboBox):
    def __init__(self, parent=None):
    ...
    def paintEvent(self, event):
        ...
        barRect = QtCore.QRect(
            0, self.height() * 0.25, (self.width()) * progress, self.height() * 0.5
        )
        painter.drawRect(barRect)

        outline = QtCore.QRect(
            1, self.height() * 0.25, (self.width()) - 2, self.height() * 0.5
        )
...
class DeleteWidget(QtWidgets.QPushButton):
    def __init__(self, parent=None):
    ...
    def paintEvent(self, event):
        ...
        (
            polygon1
            << QtCore.QPoint(self.padding, self.padding)
            << QtCore.QPoint(self.width() - self.padding, self.height() - self.padding)
        )
        polygon2 = QtGui.QPolygon()
        (
            polygon2
            << QtCore.QPoint(self.padding, self.height() - self.padding)
            << QtCore.QPoint(self.width() - self.padding, self.padding)
        )
...
class MainWindow(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.animGroupsDeleted = (
            []
        )  # HOLD ANIMATIONS FOR DELETED WIDGETS - REQUIRED FOR OVERLAPPING DELETE ACTIONS
        self.settingsFile = ""
        self.warningText = ""
    ...
    def setupUI(self):
        ...
        self.sortButton = QtWidgets.QPushButton("Reverse Sorting")
        ...
        self.hideButton = QtWidgets.QPushButton("Hide Finished Tasks")
        self.hideButton.setCheckable(True)
        self.hideButton.setToolTip("Hide finished tasks to keep the list tidy")
        self.clipboardButton = QtWidgets.QPushButton("Copy To Clipboard")
        self.clipboardButton.setToolTip(
            "Push to copy current task info to cliboard for pasting into emails or other text documents.\nHandy to keep those coordinators happy."
        )
    ...
    def createTaskWidgets(self):
        ...
        self.taskWidgets = [
            TaskWidget(t, self.taskContainer) for t in self.taskStore.tasks
        ]
        self.update()
    ...
    def setSettingsFile(self):
        ...
        if self.inNuke:
            ...
            if hieroSetup():
                self.settingsFile = [
                    tag.metadata().value("tag.settingsFile")
                    for tag in hiero.core.findProjectTags()
                    if tag.name() == "ohufx.ToDoList"
                ][0]
        else:
    ...
    def deleteTask(self):
        ...
        self.taskWidgets = [
            ...
            if self.taskWidgetToDelete != taskWidget
        ]
    ...
    def copyToClipboard(self):
        sortedTasks = sorted(
            [t for t in self.taskStore.tasks if t.index >= 0],
    ...
    def onAddTask(self):
        ...
        newTaskWidget.taskNameWidget.setSelection(
            0, len(newTaskWidget.taskNameWidget.text())
        )
        newTaskWidget.taskNameWidget.setFocus(
    ...
    def connectTaskWidgetSignals(self, taskWidget):
        ...
        taskWidget.taskNameWidget.textChanged.connect(taskWidget.task.set_name)
            ...
            if type(widget) == type(self):
                ...
                while p:
                    if p.parent() and isinstance(p.parent(), QtGui.QStackedWidget):
                        p.parent().removeWidget(
                            p
                        )  # THIS ASSUMES NUKE'S QSTACKEDWIDGET HOLDING THIS WIDGET
                        p = None
                    else:
                        p = p.parentWidget()
...
def registerNukePanel():
    ...
    nukescripts.registerWidgetAsPanel(
        "ToDoList.MainWindow", "To Do List", MainWindow.appName
    )
